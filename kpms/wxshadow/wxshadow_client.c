...
static void print_usage(const char *prog) {
    ...
    printf("  %s -p <pid> -a <addr> --once          Set one-shot BP (kernel dmesg HOOK OK, auto-disarm)\n", prog);
...
/* Set breakpoint via prctl. once!=0 => auto-disarm after first hit (kernel log). */
static int set_breakpoint(pid_t pid, unsigned long addr, int once) {
    if (run_wxshadow_prctl("SET_BP", PR_WXSHADOW_SET_BP, pid, addr,
                           once ? 1 : 0, 0) < 0)
        return -1;

    if (once)
        printf("One-shot kernel BP set at 0x%lx for pid %d\n",
               addr, target_pid(pid));
    else
        printf("Breakpoint set at 0x%lx for pid %d\n", addr, target_pid(pid));
    printf("  check: adb shell su -c 'dmesg | grep \"HOOK OK\"'\n");
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    /* Set breakpoint (optional one-shot kernel proof log) */
    if (set_breakpoint(pid, addr, do_once) < 0)
        return 1;
