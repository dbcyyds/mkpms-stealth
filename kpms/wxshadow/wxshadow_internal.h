...
static inline bool is_permission_fault(unsigned int esr)
{
     ...
     * 0x0C is "unknown 12" and is included by the (fsc & 0x3C)==0x0C test
     * for the 0x0C-0x0F nibble; keep it for back-compat.
     *
     * Include access-flag faults: some GKI 6.6 paths clear AF or re-fault
     * through AF before permission, and we still need to flip pages.
     */
    if ((fsc & 0x3C) == 0x0C)
        return true; /* 0x0C..0x0F */
    if (fsc >= 0x09 && fsc <= 0x0B)
        return true; /* access flag */
    return false;
}
...
/* Restore original insn on shadow and deactivate BP (for once-mode). */
