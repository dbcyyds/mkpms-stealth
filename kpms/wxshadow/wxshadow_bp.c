...
int wxshadow_do_set_bp_flags(void *mm, unsigned long addr, int once)
{
    struct wxshadow_write_ctx ctx;
    unsigned long offset = addr & ~PAGE_MASK;
    int ret;
    ...
    pr_info("wxshadow: [set_bp] addr=%lx once=%d\n", addr, once);
    ...
    /* Stamp once-flag on the slot we just activated */
    spin_lock(&global_lock);
    if (bp_idx >= 0 && bp_idx < ctx.page_info->nr_bps) {
        ctx.page_info->bps[bp_idx].once = once ? true : false;
    }
    spin_unlock(&global_lock);
    ...
    if (!ctx.is_new) {
        ...
        if (ret < 0)
            goto out_abort;
        pr_info("wxshadow: bp at %lx (existing page) once=%d\n", addr, once);
    ...
    if (ret == 0) {
        pr_info("wxshadow: bp at %lx orig_pfn=%lx shadow_pfn=%lx once=%d\n",
...
{
    return wxshadow_do_set_bp_flags(mm, addr, 0);
}

/*
 * Disarm a breakpoint after once-hit: restore original instruction on the
 * shadow page and mark the BP inactive so subsequent executions run clean.
 * Does not fully teardown the page (step handler may still need it).
 */
int wxshadow_disarm_bp_keep_page(void *mm, unsigned long addr)
{
    struct wxshadow_page *page_info;
    ...
    if (!orig_kaddr || !is_kva((unsigned long)orig_kaddr)) {
        wxshadow_page_put(page_info);
        return -14;
    }
    ...
    ret = write_shadow_u32(page_info, page_addr, offset, orig_insn,
                           WXSHADOW_SHADOW_FLUSH_CACHELINE, true);
    if (ret == 0)
        pr_info("wxshadow: [once] disarmed bp at %lx, restored insn=%08x\n",
                addr, orig_insn);
...
void prctl_before(hook_fargs4_t *args, void *udata)
{
    ...
    case PR_WXSHADOW_SET_BP:
        ...
        /* arg4: non-zero => once-mode (auto-disarm after first hit) */
        ret = wxshadow_do_set_bp_flags(mm, arg3, arg4 ? 1 : 0);
        kfunc_mmput(mm);
        args->ret = ret;
        args->skip_origin = 1;
        break;
