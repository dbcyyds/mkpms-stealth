...
KPM_NAME("wxshadow");
KPM_VERSION("1.2.0-k66");
KPM_LICENSE("GPL v2");
KPM_AUTHOR("wxshadow");
KPM_DESCRIPTION("W^X Shadow Memory - Hidden BP/Patch (kernel 6.1+/6.6 maple-tree)");
#endif
...
/*
 * Debug stage gate for kernel 6.6 bring-up.
 *
 * IMPORTANT: APatch often loads KPM with args="1" meaning "enabled".
 * A bare digit is therefore treated as FULL init, not a stage gate.
 *
 * Explicit debug only:
 *   s0 / stage0 = log only
 *   s1 / stage1 = resolve_symbols
 *   s2 / stage2 = + mm/vma/task scans
 *   s3 / stage3 = + BRK/step hooks
 *   s4 / stage4 = + prctl hook
 *   s5 / stage5 = + page fault
 *   s6 / stage6 = + exit_mmap
 *   empty / "1" / anything else = full init (default)
 */
static int wxshadow_parse_stage(const char *args)
{
    const char *p;
    ...
    /* args="1" from APatch enable flag, or any other string → full */
    return 9;
}
...
{
    int ret;
    int stage = wxshadow_parse_stage(args);

    pr_info("wxshadow: initializing... (stage=%d args=%s event=%s)\n",
            stage, args ? args : "(null)", event ? event : "(null)");

    if (stage <= 0) {
        pr_info("wxshadow: stage0 ok (no work)\n");
        return 0;
    }
    ...
    pr_info("wxshadow: stage1 resolve_symbols ok\n");
    if (stage <= 1)
        return 0;
    ...
    pr_info("wxshadow: stage2 scans ok (vm_mm_off=0x%x)\n", (int)vma_vm_mm_offset);
    if (stage <= 2)
        return 0;
    ...
    /* Prefer direct hook first on 6.6 if available; register API second */
    if (kfunc_brk_handler && kfunc_single_step_handler) {
        pr_info("wxshadow: using direct hook method\n");
    ...
    } else if (kfunc_register_user_break_hook && kfunc_register_user_step_hook) {
    ...
    pr_info("wxshadow: stage3 brk/step hooks ok (method=%d)\n", (int)hook_method);
    if (stage <= 3)
        return 0;
    ...
    pr_info("wxshadow: hooked prctl syscall (stage4 ok)\n");
    if (stage <= 4)
        return 0;
    ...
    pr_info("wxshadow: stage5 fault hook done (ptr=%px)\n", kfunc_do_page_fault);
    if (stage <= 5)
        return 0;
    ...
    pr_info("wxshadow: stage6 exit_mmap ok\n");
    if (stage <= 6)
        return 0;
