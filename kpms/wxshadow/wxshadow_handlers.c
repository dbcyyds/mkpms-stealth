...
/*
 * Page-fault / mem-abort hook — intercept faults for wxshadow pages.
 *
 * Linux 6.6 arm64 typically hooks do_mem_abort (do_page_fault is static):
 *   void do_mem_abort(unsigned long far, unsigned long esr, struct pt_regs *regs)
 * Older kernels may expose do_page_fault with the same 3 args (ret int).
 * FAR may carry MTE tags; strip them for page lookup.
 */
static void do_page_fault_before_impl(hook_fargs3_t *args, void *udata)
{
    unsigned long far = (unsigned long)args->arg0;
    unsigned long far_raw = far;
    unsigned int esr = (unsigned int)(unsigned long)args->arg1;
    enum wxshadow_fault_access access;
    void *mm;
    struct wxshadow_page *page;

    /* untagged user address (TBI / MTE top-byte) */
    far &= 0x00ffffffffffffffUL;
...
static int wxshadow_brk_handler_impl(struct pt_regs *regs, unsigned int esr)
{
    ...
    /*
     * Kernel-hook success signal (no userspace log / no Dobby):
     * once-mode BP logs a single clear line then disarms so hot paths
     * (e.g. render) are not broken every frame.
     */
    if (bp && bp->once) {
        pr_info("wxshadow: ========== [HOOK OK] ==========\n");
        pr_info("wxshadow: kernel hook SUCCESS once-hit pc=%lx\n", pc);
        pr_info("wxshadow: (auto-disarm; no more traps at this addr)\n");
        pr_info("wxshadow: =================================\n");
        /* Restore original insn on shadow + deactivate BP before step ends */
        (void)wxshadow_disarm_bp_keep_page(mm, pc);
    } else {
